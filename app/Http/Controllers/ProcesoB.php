...
class ProcesoB extends Controller
{
    ...
    {
        $proceso = DB::table('proceso')->select('identificador', 'nombreProceso')->where('id', 2)->first();
        ...
        return view('procesoB.index', compact('proceso', 'publicaciones','returnUrl'));
    }
    ...
    public function create()
    {
        ...
        return view('procesoB.crear', compact('tipoProcesoId'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->validate(
         ...
         $existePublicacion = Publicacion::where('proceso_id', 2)
 ...
 // Obtener el proceso actualmente seleccionado
        $proceso = 'procesoB'; 
         ...
         return redirect()->route('procesoB.index');
    }
    ...
    public function edit($id)
    {
        ...
        Breadcrumbs::for('editarPublicacionB', function ($trail) use ($publicacion) {
            $trail->parent('procesoB'); 
            $trail->push('Editar proceso', route('procesoB.edit', $publicacion->id)); 
        });

        return view('procesoB.editar', compact('publicacion', 'documentacion', 'tipoProcesoId')); //Se utiliza la función compact para pasar las variables publicacion, documentacion y tipoProcesoId a la vista
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        return redirect()->route('procesoB.index')->with('success_edit', 'La publicación y documentación se han actualizado correctamente.');
    }
    ...
    public function responder($id, $documentacionId)
    {
        ...
        Breadcrumbs::for('responderProcesoB', function ($trail) use ($id, $documentacionId) {
            $publicacion = Publicacion::findOrFail($id); 
            $documentacion = Documentacion::findOrFail($documentacionId); 
            ...
            $trail->parent('procesoB');
            $trail->push('Responder publicación', route('procesoB.responder', ['id' => $id, 'documentacionId' => $documentacionId])); 
        ...
        return view('procesoB.responder', compact('publicacion', 'documentacion', 'respuesta'));
    }
    ...
    public function calificar($id)
    {
        ...
        Breadcrumbs::for('calificarProcesoB', function ($trail) use ($publicacion) {
            $trail->parent('procesoB'); 
            $trail->push('Calificar publicación '. $publicacion->anioRegistro, route('procesoB.calificar', $publicacion->id)); 
        });

        return view('procesoB.calificar', compact('publicacion', 'documentaciones'));
    }
    ...
    public function calificarRespuesta(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Respuesta calificada exitosamente');
    }
    ...
    public function graficaDocumentacionCompletaPorAnio()
    {
        ...
        $publicaciones = Publicacion::where('proceso_id', 2)->with('documentacion.calificaciones')->get();
        ...
        return view('procesoB.graficasB', compact('anioCompletas', 'anioIncompletas', 'porcentajeCompletas', 'datosGraficaBar'));
    }
    ...
    public function generaPDF($nombreImagen1, $nombreImagen2)
    {
        ...
        return $pdf->download('Reporte_Proceso_B.pdf');
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            foreach ($documentaciones as $documentacion) {
                ...
                // Si hay un archivo adjunto, elimína del almacenamiento
            ...
            return redirect()->route('procesoB.index')->with('success', 'La publicación y sus relaciones han sido eliminadas con éxito.');
        }  catch (\Exception $e) {
            // Manejo de errores
            return redirect()->route('procesoB.index')->with('error', 'Ha ocurrido un error al eliminar la publicación.');
        }
    }
}