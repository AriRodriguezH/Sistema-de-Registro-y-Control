...
class ProcesoE extends Controller
{
    ...
    {
        $proceso = DB::table('proceso')->select('identificador', 'nombreProceso')->where('id', 5)->first();
        ...
        return view('procesoE.index', compact('proceso', 'publicaciones','returnUrl'));
    }
    ...
    public function create()
    {
        ...
        return view('procesoE.crear', compact('tipoProcesoId'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $existePublicacion = Publicacion::where('proceso_id', 5)
        ...
        return redirect()->route('procesoE.index');
    }
    ...
    public function edit(string $id)
    {
        ...
        Breadcrumbs::for('editarPublicacionE', function ($trail) use ($publicacion) {
            $trail->parent('procesoE'); 
            $trail->push('Editar proceso', route('procesoE.edit', $publicacion->id)); 
        });

        return view('procesoE.editar', compact('publicacion', 'documentacion', 'tipoProcesoId')); //Se utiliza la función compact para pasar las variables publicacion, documentacion y tipoProcesoId a la vista
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $this->validate(
         ...
         return redirect()->route('procesoE.index')->with('success_edit', 'La publicación y documentación se han actualizado correctamente.');
    ...
    public function responder($id, $documentacionId)
    {
        ...
        Breadcrumbs::for('responderProcesoE', function ($trail) use ($id, $documentacionId) {
            $publicacion = Publicacion::findOrFail($id); 
            $documentacion = Documentacion::findOrFail($documentacionId); 
            ...
            $trail->parent('procesoE');
            $trail->push('Responder publicación', route('procesoE.responder', ['id' => $id, 'documentacionId' => $documentacionId])); 
        ...
        return view('procesoE.responder', compact('publicacion', 'documentacion', 'respuesta'));
    }
    ...
    public function calificar($id)
    {
        ...
        Breadcrumbs::for('calificarProcesoE', function ($trail) use ($publicacion) {
            $trail->parent('procesoE'); 
            $trail->push('Calificar publicación '. $publicacion->anioRegistro, route('procesoE.calificar', $publicacion->id)); 
        });

        return view('procesoE.calificar', compact('publicacion', 'documentaciones'));
    }
    ...
    public function graficaDocumentacionCompletaPorAnio()
    {
        ...
        $publicaciones = Publicacion::where('proceso_id', 5)->with('documentacion.calificaciones')->get();
        ...
        return view('procesoE.graficasE', compact('anioCompletas', 'anioIncompletas', 'porcentajeCompletas', 'datosGraficaBar'));
    }
    ...
    public function generaPDF($nombreImagen1, $nombreImagen2)
    {
        ...
        return $pdf->download('Reporte_Proceso_E.pdf');
    }
    ...
    public function destroy(string $id)
    {
        try {
            ...
            return redirect()->route('procesoE.index')->with('success', 'La publicación y sus relaciones han sido eliminadas con éxito.');
        }  catch (\Exception $e) {
            // Manejo de errores
            return redirect()->route('procesoE.index')->with('error', 'Ha ocurrido un error al eliminar la publicación.');
        }
    }
}