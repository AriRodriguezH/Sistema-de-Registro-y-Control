...
class ProcesoD extends Controller
{
    ...
    {
        $proceso = DB::table('proceso')->select('identificador', 'nombreProceso')->where('id', 4)->first();
        ...
        if ($user->hasRole('Administrador')) {
            ...
            //obtiene todas las publicaciones que han sido creadas por el usuario administrador actual.
            /*filtrar las publicaciones que tengan el mismo users_id que el usuario actualmente autenticado
       lo hará con  el método where('users_id', $user->id)*/
            $publicaciones = Publicacion::where('users_id', $user->id)
                ->with('documentacion')
                ->get();
        } else {
        ...
        return view('procesoD.index', compact('proceso', 'publicaciones','returnUrl'));
    }
    ...
    public function create()
    {
        ...
        return view('procesoD.crear', compact('tipoProcesoId'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $existePublicacion = Publicacion::where('proceso_id', 4)
        ...
        return redirect()->route('procesoD.index');
    }
    ...
    public function edit(string $id)
    {
        ...
        Breadcrumbs::for('editarPublicacionD', function ($trail) use ($publicacion) {
            $trail->parent('procesoD'); 
            $trail->push('Editar proceso D', route('procesoD.edit', $publicacion->id)); 
        });

        return view('procesoD.editar', compact('publicacion', 'documentacion', 'tipoProcesoId')); //Se utiliza la función compact para pasar las variables publicacion, documentacion y tipoProcesoId a la vista
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $this->validate(
         ...
         return redirect()->route('procesoD.index')->with('success_edit', 'La publicación y documentación se han actualizado correctamente.');
    ...
    public function responder($id, $documentacionId)
    {
        ...
        Breadcrumbs::for('responderProcesoD', function ($trail) use ($id, $documentacionId) {
            $publicacion = Publicacion::findOrFail($id); 
            $documentacion = Documentacion::findOrFail($documentacionId); 
            ...
            $trail->parent('procesoD');
            $trail->push('Responder proceso D', route('procesoD.responder', ['id' => $id, 'documentacionId' => $documentacionId])); 
        ...
        return view('procesoD.responder', compact('publicacion', 'documentacion', 'respuesta'));
    }
    ...
    public function calificar($id)
    {
        ...
        Breadcrumbs::for('calificarProcesoD', function ($trail) use ($publicacion) {
            $trail->parent('procesoD'); 
            $trail->push('Calificar proceso D '. $publicacion->anioRegistro, route('procesoD.calificar', $publicacion->id)); 
        });

        return view('procesoD.calificar', compact('publicacion', 'documentaciones'));
    }
    ...
    public function calificarRespuesta(Request $request)
    {
        ...
        // Realizar la lógica de calificación aquí...
    ...
    public function graficaDocumentacionCompletaPorAnio()
    {
        ...
        $publicaciones = Publicacion::where('proceso_id', 4)->with('documentacion.calificaciones')->get();
        ...
        return view('procesoD.graficasD', compact('anioCompletas', 'anioIncompletas', 'porcentajeCompletas', 'datosGraficaBar'));
    }
    ...
    public function generaPDF()
    {
        ...
        $viewData = [
            ...
            'promedioCumplimientoPrimerSemestre' => $promedioCumplimientoPrimerSemestre, // Pasa los promedios
            'promedioCumplimientoSegundoSemestre' => $promedioCumplimientoSegundoSemestre, // Pasa los promedios
        ];


        $pdf = PDF::loadView('usuarios.reporteProceso', $viewData);

        return $pdf->download('Reporte_Proceso_D.pdf');
    }
