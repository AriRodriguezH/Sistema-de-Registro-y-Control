...
<div class="d-flex justify-content-between align-items-center">
    <h4 class="font-weight-bold text-center mx-auto">Responder publicación A</h4>
    <a href="{{ route('procesoA.index', ['return_url' => url()->current()]) }}" class="btn" style="background-color: #AAADAD; color: #FFFFFF; padding-left: 10px;" tabindex="6">
        <i class="fas fa-chevron-left" style="margin-right: 5px;"></i>
        Volver
    </a>
</div>
<br>
{{ Breadcrumbs::render('responderProcesoA') }}
@stop
...
<div class="card">
    <div class="card-body">
        ...
        <table class="table table-bordered table-responsive-sm table-bordered mt-4">
            ...
            <tr>
                <td>
                    ...
                    @php
                    $fileName = pathinfo($archivoPath, PATHINFO_FILENAME);
                    @endphp
                    <a href="{{ asset('storage/' . $archivoPath) }}" target="_blank">{{ $fileName }}</a>
                    <br>
                    @endforeach
                </td>
                <td>
                    @if (count(explode('|', $respuesta->archivo_path)) > 1)
                    <a href="{{ route('procesoA.descargarArchivos', ['id' => $respuesta->id]) }}">Descargar archivos (ZIP)</a><br>
                    @else
                    @foreach(explode('|', $respuesta->archivo_path) as $archivoPath)
        ...
        <div class="modal fade" id="editarRespuestaModal" tabindex="-1" role="dialog" aria-labelledby="editarRespuestaModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content ">
                    ...
                    {!! Form::open(['route' => ['procesoA.actualizarRespuesta', $respuesta->id, $documentacion->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data']) !!}
        ...
        <h5>Responder:</h5>
        {!! Form::open(['route' => ['procesoA.guardarRespuesta', 'documentacionId' => $documentacion->id], 'method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
        @csrf
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <br>
            <label id="archivosSeleccionados" class="mt-2" style="font-size: 13px;"></label> <!-- Para mostrar los nombres de los archivos seleccionados -->
        </div>
