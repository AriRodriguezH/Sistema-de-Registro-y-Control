...
<div class="row">
    <div class="col-md-3">
        <div class="form-group">
            @can('crear-procesos')
            <a class="btn btn-primary" href="{{ route('procesoH.create') }}"><i class="fas fa-plus-square"></i> <i style="padding-left: 5px;"></i>Nuevo</a>
            @endcan
        </div>
    </div>
    <div class="col-md-9">
...
<div class="card">
    <div class="card-body">
        ...
        <div class="row">
            ...
            <div class="col-md-12 card-container"> <!-- Agregar la clase card-container aquí -->

                @if(Auth::user()->hasRole('Administrador'))
                @foreach($publicaciones as $publicacion)
                @if($publicacion->proceso_id === 8)
                <div class="card card-success collapsed-card mb-4">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            <a href="{{ route('procesoH.editar', $publicacion->id) }}" class="btn btn-tool">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a href="{{ route('procesoH.calificar', ['id' => $publicacion->id]) }}" class="btn btn-tool">
                            ...
                            <form id="delete-form-{{$publicacion->id}}" action="{{ route('procesoH.destroy', $publicacion->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
                        ...
                        <ul>
                            ...
                            <ul>
                                <li>{{ $documentacion->descripcion }}
                                    @if(!Auth::user()->hasRole('Administrador'))
                                    <br>
                                    <a href="{{ route('procesoH.responder', ['id' => $publicacion->id, 'documentacionId' => $documentacion->id]) }}">Ver Asignación</a>
